...
alter table claims add column if not exists visitor_id uuid;
alter table claims add column if not exists insurer_id uuid;
alter table claims add column if not exists prescription varchar(2000);

create index if not exists idx_claims_visitor_id on claims (visitor_id);
